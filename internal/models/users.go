	...
	if err != nil {
		...
		if errors.As(err, &pgError) {
			...
			if pgError.Code == "23505" && strings.Contains(pgError.Message, "users_uc_email") {
				return ErrDuplicateEmail
				// 23505
				// duplicate key value violates unique constraint "users_uc_email"
				// fmt.Println("&pgError.Code --> ", pgError.Code)
				// fmt.Println("&pgError.Message --> ", pgError.Message)
			}
		}
		return err
	}
