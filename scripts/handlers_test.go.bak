...
func TestPing2(t *testing.T) {
	// Create a new instance of our application struct. For now, this just contains a structured logger (which discards anything written to it).
	app := &application{
		logger: slog.New(slog.NewTextHandler(io.Discard, nil)),
	}

	// We then use the httptest.NewTLSServer() function to create a new test server, passing in the value returned by our app.routes() method as the handler for the server. This starts up a HTTPS server which listens on a randomly-chosen port of your local machine for the duration of the test. Notice that we defer a call to ts.Close() so that the server is shutdown when the test finishes.
	ts := httptest.NewTLSServer(app.routes())
	defer ts.Close()

	// The network address that the test server is listening on is contained in the ts.URL field. We can  use this along with the ts.Client().Get() method to make a GET /ping request against the test server. This returns a http.Response struct containing the response.
...
// 	// Log the CSRF token value in our test output using the t.Logf() function. The t.Logf() function works in the same way as fmt.Printf(), but writes the provided message to the test output.
// 	t.Logf("CSRF token is: %q", csrfToken)
// }

func TestUserSignup2(t *testing.T) {
	app := newTestApplication(t)
	ts := newTestServer(t, app.routes())
	defer ts.Close()
	...
	for _, tt := range tests[7:] {
		t.Run(tt.name, func(t *testing.T) {
			...
			// code, headers, body := ts.postForm(t, "/user/signup", form)
			code, _, body := ts.postForm(t, "/user/signup", form)

			// fmt.Println("tt.csrfToken --> ", tt.csrfToken)
			// fmt.Println("TEST FORM --> ", form)
...
func TestSnippetCreate(t *testing.T) {
	...
	t.Run("Authenticated", func(t *testing.T) {
		...
		form := url.Values{}
		form.Add("email", "usern1@gmail.com")
		form.Add("password", "usern1@gmail.com")
